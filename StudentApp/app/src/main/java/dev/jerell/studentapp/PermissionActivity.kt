...
class PermissionActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!hasAllPermissions()) {
            ...
            if (SDK_INT >= 33) {
                perm.add(Manifest.permission.NEARBY_WIFI_DEVICES)
            }

            ActivityCompat.requestPermissions(this, perm.toTypedArray(), requestCode)
        } else {
            navigateToNextPage()
        }
    }
    ...
    private fun hasAllPermissions(): Boolean {
        ...
        // Check NEARBY_WIFI_DEVICES permission for Android 13+
        if (SDK_INT >= 33) {
        ...
        when (requestCode) {
            this.requestCode -> {
                if (hasAllPermissions()) {
                    ...
                    // Optionally, you can add a check here to guide the user to enable permissions if they deny
                }
            }
    ...
    // This method guides the user to the app's settings if permissions are denied.
